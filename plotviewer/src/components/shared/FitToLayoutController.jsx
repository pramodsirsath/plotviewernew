...
export default function FitToLayoutController({
  fitKey,
  isTopDown,
  image,
  layout,
  scale = 0.05,
  duration = MIN_HOME_DURATION,
  onStart,
  onComplete
}) {
  ...
  useEffect(() => {
    ...
    // 🔥 ONLY CHANGE: MOBILE DISTANCE FIX
    const baseDist = Math.max(distForH, distForW);

    const dist = isMobile
      ? baseDist * 1.8   // 🔥 mobile zoom fix (IMPORTANT)
      : baseDist * 1.2;
    ...
    try {
      ...
      startAzimuth = startSpherical.theta;
    }
    ...
    const animate = (now) => {
      ...
      const currentAzimuth =
        startAzimuth + shortestAngleDiff(startAzimuth, endAzimuth) * straightenEase;
